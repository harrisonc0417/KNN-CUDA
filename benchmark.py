...
def _make_regression(
    ...
    def _data(n):
        ...
        y = (np.sin(X @ proj) + rng.standard_normal(n) * 0.05).astype(np.float32)
        return X, y
...
def bench_classification(
    ...
    print(f"Classification  n_train={n_train:,}  n_query={n_query:,}"
          f"  d={n_features}  k={k}  classes={n_classes}")
    print(f"{'─'*60}")

    train_X, train_y, query_X, query_y = _make_blobs(
        n_train, n_query, n_features, n_classes, seed)
...
def bench_regression(
    ...
    train_X, train_y, query_X, query_y = _make_regression(
        n_train, n_query, n_features, seed)
    ...
    results.append(BenchResult("CUDA KNN", t_cuda, mse=mse_cuda))
    ...
    t0       = time.perf_counter()
    reg_sk.fit(train_X, train_y)
    preds_sk = reg_sk.predict(query_X)
    t_sk     = time.perf_counter() - t0
    mse_sk   = float(np.mean((preds_sk - query_y) ** 2))
    results.append(BenchResult("sklearn (brute, CPU)", t_sk, mse=mse_sk))
